...

{
  ...
  launch_insgorund::
    ...
    then
      ...
      '/go/bin/insgorund -l 127.0.0.1:18181 --rpc 127.0.0.1:18182 -d /tmp/code',
  launch_insolard::
    if params.insolar.local_launch == true
    then
      '/go/bin/insolard --config /opt/insolar/config/node-insolar.yaml ' + self.enable_trace + ' 2>&1 | tee /logs/$(POD_NAME).insolard.log 2>&1'
    else
      '/go/bin/insolard --config /opt/insolar/config/node-insolar.yaml ' + self.enable_trace,
        ...
        containers: [
          {
            ...
            tty: true,
            stdin: true,
            command: std.split($.launch_insgorund, ' '),
            env: [
              {
                name: 'HOME',
                value: '/opt/insolar',
          ...
          },
          {
            ...
            tty: true,
            stdin: true,
            command: std.split($.launch_insolard, ' '),
            env: [
              {
                name: 'HOME',
                value: '/opt/insolar',
