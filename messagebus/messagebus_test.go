...
func TestMessageBus_SendViaWatermill(t *testing.T) {
	...
	inMessages, err := pubsub.Subscribe(context.Background(), bus.ExternalMsgTopic)
	require.NoError(t, err)

	mb, _, _ := messageBusMock(t, ctx, 100, 100)
	mb.pub = pubsub
	go func(ctx context.Context, messages <-chan *wmMessage.Message) {
		for msg := range messages {
			replyMsg := wmMessage.NewMessage(msg.UUID, reply.ToBytes(&reply.OK{}))
